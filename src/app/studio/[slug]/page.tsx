...
function SortableCategoryCard({
  cat, stats, product, onToggleVisibility, onRemoveFromFolder, onDelete, onRename,
}: {
  cat: string;
  ...
  function commitRename() {
    ...
    setEditing(false);
    if (trimmed && trimmed !== cat) onRename?.(trimmed);
    else setEditName(cat);
  }
      ...
      <div className="flex items-center gap-4">
        <div className={`w-11 h-11 rounded-xl ${c.iconBg} flex items-center justify-center flex-shrink-0`}>
          <svg className={`w-5 h-5 ${c.iconText}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.75} d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A2 2 0 013 12V7a2 2 0 012-2h2z" />
          </svg>
        </div>
            ...
            {editing ? (
              <input
                ...
                onKeyDown={(e) => {
                  ...
                  if (e.key === "Escape") { setEditing(false); setEditName(cat); }
                }}
                onClick={(e) => e.stopPropagation()}
                className="font-semibold text-gray-900 text-sm w-full border-b border-blue-500 outline-none bg-transparent leading-tight"
              />
            ) : (
              <h2 className="font-semibold text-gray-900 group-hover:text-blue-600 transition-colors truncate leading-tight">{cat}</h2>
            )}
            {!editing && complete && <span className="text-sm flex-shrink-0">✅</span>}
            {!editing && stats.drafts > 0 && (
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-10 right-1 z-20 flex opacity-0 group-hover/drag:opacity-100 transition-opacity">
        {onRename && (
          <button
            onClick={(e) => { e.preventDefault(); e.stopPropagation(); setEditing(true); setEditName(cat); }}
      ...
      {editing ? (
        <div className="relative block bg-white rounded-2xl border border-blue-400 px-5 pb-5 pt-8 shadow-sm">
          {cardContent}
        </div>
      ) : (
        <Link
...
export default function StudioProductPage() {
  ...
  async function handleRenameCategory(oldName: string, newName: string) {
    ...
    const updated = checklist.map((i) => {
      ...
      return cat === oldName ? { ...i, category: newName } : i;
    });
    setChecklist(updated);
    fetch("/api/checklist", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ type: "reorder", productId: slug, items: updated }),
    });
    const newOrder = catMeta.order.map((c) => c === oldName ? newName : c);
    const newAssignment = { ...catMeta.folderAssignment };
    if (newAssignment[oldName] !== undefined) {
      newAssignment[newName] = newAssignment[oldName];
      delete newAssignment[oldName];
    }
    const nextMeta = { ...catMeta, order: newOrder, folderAssignment: newAssignment };
    setCatMeta(nextMeta);
    persistMeta(nextMeta);
    if (product?.categoryVisibility?.[oldName] !== undefined) {
      const newCatViz = { ...product.categoryVisibility };
      newCatViz[newName] = newCatViz[oldName];
      delete newCatViz[oldName];
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {searchResults.map((cat) => (
                <SortableCategoryCard
                  ...
                  onRename={(n) => handleRenameCategory(cat, n)}
